...
function BookRide() {
  ...
  useEffect(() => {
    const fetchRideDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/v1/book-ride/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3000/api/v1/create-passengerRide/${id}`, formDataToSend);
    ...
    <div className="p-4 max-w-md mx-auto">
      ...
      {submitted ? (
        ...
        <div className="mt-4 text-center">
          <p className="text-lg text-green-600 font-semibold">Thank you! Your ride has been booked.</p>
          <button
            onClick={() => navigate("/")}
